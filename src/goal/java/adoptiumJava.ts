...
function transformRuntime(entry: AdoptiumJavaRuntimeEntry): VersionFileRuntime[] {
	...
	const name = `${entry.vendor}_termurin_jre${entry.version_data.major}.${entry.version_data.minor}.${entry.version_data.security}${entry.version_data.patch ? `.${entry.version_data.patch}` : ""}+${entry.version_data.build}`;
